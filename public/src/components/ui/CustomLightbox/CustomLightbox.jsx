...
exports.default = CustomLightbox;
const react_1 = require("react");
function CustomLightbox({ items, initialIndex, onClose, }) {
    const [index, setIndex] = (0, react_1.useState)(initialIndex);
    const [zoom, setZoom] = (0, react_1.useState)(0.4); // 40% par défaut
    const [targetZoom, setTargetZoom] = (0, react_1.useState)(0.4);
    const [pan, setPan] = (0, react_1.useState)({ x: 0, y: 0 });
    const [isDragging, setIsDragging] = (0, react_1.useState)(false);
    const imageRef = (0, react_1.useRef)(null);
    const animationFrame = (0, react_1.useRef)(null);
    const current = items[index];
    const goPrev = (0, react_1.useCallback)(() => setIndex((i) => (i - 1 + items.length) % items.length), [items.length]);
    const goNext = (0, react_1.useCallback)(() => setIndex((i) => (i + 1) % items.length), [items.length]);
    ...
    return (<div className="bg-bgcontainer fixed inset-0 z-[9999] flex flex-col dark:text-white" onClick={(e) => e.target === e.currentTarget && onClose()}>
      {/* Header */}
      <div className="relative z-10 flex items-center justify-between bg-bghcontainer px-6 py-4 backdrop-blur-md">
        <div className="flex items-center gap-3">
          <button onClick={onClose} className="rounded-full bg-white/40 p-3 transition hover:bg-white/60" aria-label="Fermer">
        ...
        <div className="flex items-center gap-1 sm:gap-3">
          <div className="flex items-center gap-1 sm:gap-2 rounded-full bg-white/10 px-4 py-2">
            ...
            <button onClick={() => handleButtonZoom(-0.2)} className="rounded p-1 transition hover:bg-white/20" aria-label="Zoom arrière">
              <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button onClick={() => handleButtonZoom(0.2)} className="rounded p-1 transition hover:bg-white/20" aria-label="Zoom avant">
              <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button onClick={centerImage} className="rounded p-1 transition hover:bg-white/20" aria-label="Recentrer" title="Recentrer l’image">
              <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 12a4 4 0 100-8 4 4 0 000 8z"/>
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 2v2m0 16v2m10-10h-2M4 12H2m15.364 6.364l-1.414-1.414M6.05 6.05l-1.414-1.414m0 12.728l1.414-1.414m12.728 0l1.414-1.414"/>
              </svg>
            </button>
          </div>

          {items.length > 1 && (<button onClick={goPrev} className="rounded-full bg-white/20 p-3 transition hover:bg-white/40" aria-label="Précédent">
              <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7"/>
              </svg>
            </button>)}

          {items.length > 1 && (<button onClick={goNext} className="rounded-full bg-white/20 p-3 transition hover:bg-white/40" aria-label="Suivant">
              <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="relative flex-1 overflow-hidden">
        <div className="absolute inset-0 overflow-auto">
          <div className="flex h-full w-full items-center justify-center">
            ...
            <img ref={imageRef} src={current.full} alt={current.caption} draggable={false} className="max-w-none select-none" style={{
            transform: `translate(${pan.x}px, ${pan.y}px) scale(${zoom})`,
            transition: isDragging
                ? "none"
                : "transform 0.08s cubic-bezier(0.25, 0.8, 0.25, 1)",
            cursor: isDragging ? "grabbing" : zoom > 1 ? "grab" : "default",
        }} onMouseDown={handleMouseDown} onDoubleClick={resetZoomAndPan}/>
          </div>
        </div>
      </div>
      ...
      <div className="shrink-0 bg-black/80 px-6 py-4">
        <div className="mb-4 text-center">
          {current.caption && (<p className="mx-auto max-w-3xl truncate px-4 text-sm opacity-90">
              {current.caption}
            </p>)}
        </div>

        {items.length > 1 && (<div className="scrollbar-thin scrollbar-thumb-white/30 overflow-x-auto">
            <div className="flex min-w-max gap-4 py-2">
              {items.map((item, i) => (<button key={i} onClick={() => setIndex(i)} className={`shrink-0 overflow-hidden rounded-lg border-4 transition-all ${i === index
                    ? "scale-110 border-white opacity-100"
                    : "border-transparent opacity-60 hover:opacity-90"}`}>
                  {/* eslint-disable-next-line @next/next/no-img-element -- Required for zoom/pan functionality in lightbox */}
                  <img src={item.src} alt="" className="h-20 w-20 object-cover" loading="lazy"/>
                </button>))}
            </div>
          </div>)}
      </div>
