...
exports.JournalSommaire = void 0;
const react_1 = require("react");
const JournalSommaire = ({ sommaire, className = "", }) => {
        ...
        <ul className="space-y-3 border-l-2 border-gray-200 dark:border-gray-700">
          {hasLinks ? (<>
              ...
              <li className="ml-4 -indent-4">
                <a href="#top" onClick={scrollToTop} className="block py-1 pl-4 text-sm text-gray-500 transition-all duration-200 hover:translate-x-1 hover:text-one">
                  ↑ Retour en haut
                </a>
              </li>
              ...
              {sommaire.map((item, index) => {
                ...
                return (<li key={item.id + index} className="m-0">
                    <a href={`#${item.id}`} className={`block py-1 pl-4 text-sm transition-all duration-300 ${isActive
                        ? "font-bold text-one translate-x-2"
                        : "text-gray-600 hover:translate-x-2 hover:text-one hover:font-bold"}`} onClick={(e) => {
                        e.preventDefault();
              ...
              {/* Aller en bas */}
              <li className="ml-4 -indent-4">
                <a href="#bottom" onClick={scrollToBottom} className="block py-1 pl-4 text-sm text-gray-500 transition-all duration-200 hover:translate-x-1 hover:text-one">
                  ↓ Aller en bas de page
                </a>
              </li>
            </>) : (sommaire.map((html, index) => {
            const text = extractText(html);
            return (<li key={index} className="ml-4 py-1 pl-4 text-sm text-gray-500 dark:text-gray-500">
                  {text}
                </li>);
        }))}
        </ul>
...
exports.JournalSommaire = JournalSommaire;
